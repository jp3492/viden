...
export default function ( state = initialState, action ){
  switch (action.type) {
    case SUBMIT_HIGHLIGHT:                const { _id, videos, title, _uid } = state.selectedHighlights;
                                          const filteredHigh = action.payload.h.filter( h => {
                                            if (h.comment.includes(action.payload.s)) { return true }
                                            return false;
                                          });
                                          const selectedHighlights = { _uid, _id, videos, title, highlights: action.payload.h };
                                          return { ...state, selectedHighlights, filteredHighlights: filteredHigh };
    case FETCH_HIGHLIGHTS:                return { ...state, list: action.payload };
    case SELECT_HIGHLIGHTS:               console.log(action.payload);
                                          return { ...state, selectedHighlights: action.payload };
    case CHANGE_SEARCH:                   const filteredHighlights = state.selectedHighlights.highlights.filter( h => {
